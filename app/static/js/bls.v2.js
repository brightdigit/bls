    ...
    $.get('available', function (data) {
      for (var index = 0; index < data.length; index++) {
        if (!bls.availability.item_groups[data[index].group_name]) {
          ...
          bls.availability.item_map[data[index].group_name] = [];
        }
        ...
        if (bls.availability.item_map[data[index].group_name].indexOf(data[index].area_group_name) < 0) {
          bls.availability.item_map[data[index].group_name].push(data[index].area_group_name);
        }
      ...
      var area_groups = $('[name=area_group]');
      var item_type = $('[name=item_type]');
      for (var key in bls.availability.item_groups) {
        $('<option>').appendTo(item_type).text(key).val(key);
      }
      item_type.change( function () {
        var len = bls.availability.item_map[item_type.val()].length;
        area_groups.empty();
        for (var index = 0; index < len; index++) {
          var value = bls.availability.item_map[item_type.val()][index];
          $('<option>').appendTo(area_groups).text(value).val(value);
        }
        if (len > 1) {
          area_groups.removeAttr('disabled');
        } else {
          area_groups.attr('disabled', 'disabled');
        }
      });
    });
